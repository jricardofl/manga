...
library(lubridate)
dezembro <- as.Date("2021-12-01")
anterior <-  as.Date("2022-01-01")
atual <-  as.Date("2022-02-01")
mesatual <-  as.Date("2022-01-01")
data <- as.Date("2022-03-11") #ajustar semanalmente
sem_ano <- 10 #ajustar semanalmente
...
### Evolução da Area com Manga no Vale do São Francisco: 2016:2020 {.no-title}
...
### Evolução do Volume com Manga no Vale do São Francisco: 2016-2020 {.no-title}
...
### Evolução da Produtividade com Manga no Vale do São Francisco: 2016-2020 {.no-title}
...
### Evolução do Valor Bruto da Produção com Manga no Vale do São Francisco: 2016-2020  {.no-title}
...
### pela mangicultura em `r strftime(mesatual, format = "%B")` de 2022 
``` {r emp1}
num <- 516
valueBox(
...
### Saldo Gerado pela mangicultura em `r strftime(mesatual, format = "%B")` de 2022 
``` {r emp3}
num <- -226
valueBox(
...
g4 <- ggplot(data = subclasse_positivos, aes(x = grau_instrucao, y = ..count..)) +
  geom_bar(fill='#1e4971')+
    scale_y_continuous(limits=c(0, 200), n.breaks = 10, expand = expansion(add=c(0,0.5)))+
