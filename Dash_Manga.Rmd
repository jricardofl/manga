...
data <- as.Date("2022-02-25")
sem_ano <- 8 #ajustar semanalmente
...
manga_mespeba$mes <- seq(as.Date('2017-01-01'),to=atual,by='1 month')
...
### Volume Comercializado em `r strftime(atual, format = "%B")` de 2022 
``` {r v1}
num <- tail(round(manga_mespeba$toneladas,0),1)
valueBox(
  value = paste0(num, " Toneladas"),
  color = "tomato",
  icon= "fa-arrow-down")
...
### Volume Comercializado em `r strftime(atual, format = "%B")` de 2021 
``` {r v2}
num <- 12466
valueBox(
  value = paste0(num, " Toneladas"),
...
### Volume Comercializado em `r strftime(atual, format = "%B")` de 2020 
``` {r v3}
num <- 9432
valueBox(
  value = paste0(num, " Toneladas"),
...
g1 <- ggplot()+
  ...
  labs(y= "Toneladas", x= "Anos", title='Volume total comercializado de manga: 2017 a 2022',
...
g3 <- ggplot(data=manga_mespeba, aes(x=mes)) +  #estetica vai valer para todos os geom's
  ...
  scale_colour_manual("",
                      ...
                      values = c("blue", "red")) +
  labs(y= "Quantidade (T)", x= "Meses do Ano", title='Tendência do Volume comercializado de manga',
...
g4 <- ggplot(data=sazonalpeba_graph)+
  ...
  scale_x_date(date_breaks = "1 month",
               labels = date_format("%b"))+
  labs(y= "", x= "Meses de cada Ano", title='Sazonalidade do Volume comercializado de manga',
...
g5 <- ggplot()+
  ...
  scale_x_date(date_breaks = "1 month",
               labels = date_format("%B"))+
  labs(y= "Toneladas", x= "Meses do Ano", title='Estatísticas sobre a Evolução da Oferta de manga',
       caption = "")+
  theme_minimal()+
  theme(axis.text.x=element_text(angle=35, hjust=0.5, size=8, margin = margin(b=20)),
        axis.text.y=element_text(hjust=0.5, size=8, margin = margin(l=20)),
...
g2 <- ggplot()+
  ...
  scale_x_date(date_breaks = "1 month",
               labels = date_format("%B"))+
  labs(y= "Variação Percentual (%)", x= "Meses do Ano", title='Variação do Volume comercializado de manga',
       caption = "Fonte: Observatório de Mercado de Manga da Embrapa")+
  theme_minimal()+
  theme(axis.text.x=element_text(angle=35, hjust=0.5, size=8, margin = margin(b=20)),
        axis.text.y=element_text(hjust=0.5, size=8, margin = margin(l=20)),
