...
exporta_manga_valor_22 <- as.matrix(tail(exporta_manga_valor,2)) #ajustar mensalmente
...
exporta_manga_volume_22 <- as.matrix(tail(exporta_manga_volume,2)) #ajustar mensalmente
...
for(i in 1:2){
  variacao_volume_2022[i,1] = variacao_volume_22[i,1] #ajustar mensalmente
}
...
O objetivo desta análise foi verificar o comportamento das exportações de manga do Brasil em janeiro e fevereiro de 2022. Os dados utilizados são mensais e disponibilizados pelo **Comexstat (MDIC)** para o período de *janeiro de 2012 a `r strftime(atual, format = "%B de %Y")`*;

Em relação aos volumes, em `r strftime(atual, format = "%B")` foram de 9,245 mil toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 5,715 mil t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 7,563 mil t e, em `r strftime(atual, format = "%B")` de 2021, foram 7,712 mil t, ou seja, houve um aumento de 22,24% em relação a 2020 e de 19,90% na comparação com o mesmo período do ano passado. Se for feita uma conversão para contêineres, o volume exportado em `r strftime(atual, format = "%B de %Y")` é de cerca de `r round((tail(dados2[,4],1))/22216,0)` contêineres.  

Em termos dos valores da exportações, em `r strftime(atual, format = "%B")` foram de 8,266 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 6,351 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 7,369 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 7,952 milhões, ou seja, houve um aumento nos valores recebidos do exterior de 12,17% em relação a 2020 e de 3,95% na comparação com o mesmo período de 2021.   
